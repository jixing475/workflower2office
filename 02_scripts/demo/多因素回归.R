...
dat <- select(TMAO_1_4_5,one_of("TMAO","age","sex","hypertension","CHOL","eGFR","Cardiothoracic_ratio","heart_ultrasound"))
gg_na(dat)
...
fit.1 <- glm(TMAO ~Cardiothoracic_ratio+age+sex+CHOL,data = dat) 
fit.2 <- glm(TMAO ~Cardiothoracic_ratio+age+sex+CHOL+hypertension,data = dat)
fit.3 <- glm(TMAO ~Cardiothoracic_ratio+age+sex+CHOL+eGFR,data = dat)
fit.4 <- glm(TMAO ~Cardiothoracic_ratio+age+sex+CHOL+hypertension+eGFR,data = dat)


(res.1 <- regression_result("fit.1"))
(res.2 <- regression_result("fit.2"))
(res.3 <- regression_result("fit.3"))
(res.4 <- regression_result("fit.4"))
...
##                                                  ##
##**************************************************##
fit.UCG <- glm(TMAO ~heart_ultrasound+age+sex+CHOL+hypertension+eGFR,data = dat)
