##
TMAO <- TMAO_1_4_5[!is.na(TMAO_1_4_5$Cardiothoracic_ratio),]
fac <- TMAO$Cardiothoracic_ratio
##
...
TMAO %>%
  filter(get("Cardiothoracic_ratio") == levels(fac)[1]) %>%
  get_num() %>%
  sapply(get_stat_num) -> b#summary for group_1
TMAO %>%
  filter(get("Cardiothoracic_ratio") == levels(fac)[2]) %>%
  get_num() %>%
...
TMAO %>%
  get_fac() %>%
  select(-get("Cardiothoracic_ratio")) %>%
