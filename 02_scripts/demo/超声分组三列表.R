##
TMAO <- TMAO_1_4_5[!is.na(TMAO_1_4_5$heart_ultrasound),]
fac <- TMAO$heart_ultrasound
##
...
TMAO %>%
  filter(get("heart_ultrasound") == levels(fac)[1]) %>%
  get_num() %>%
  sapply(get_stat_num) -> b#summary for group_1
TMAO %>%
  filter(get("heart_ultrasound") == levels(fac)[2]) %>%
  get_num() %>%
